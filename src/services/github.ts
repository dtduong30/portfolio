...
export async function getProcessedProjects(
  ...
  for (const repo of repos) {
    ...
    const featuredConfig = portfolioConfig.featuredProjects.find(
      (p) => p.repo === repo.name,
    );
...
export async function getStats(
  ...
  const totalRepos = Object.values(languageTotals).reduce(
    (sum, count) => sum + count,
    0,
  );
...
export async function getReplayStats(
  ...
  try {
    ...
    const mostActiveMonthIndex = monthlyCommits.indexOf(
    ...
    pushEvents.forEach((event) => {
      ...
      dailyActivity.set(
        date,
        (dailyActivity.get(date) || 0) + (event.payload.commits?.length || 1),
      );
    });
    ...
    // Language breakdown — fetch all repo languages in parallel
    const languageTotals: { [key: string]: number } = {};
    ...
    const mostProductiveDayIndex = dayOfWeekCommits.indexOf(
      Math.max(...dayOfWeekCommits),
    );
    const dayNames = [
      "Sunday",
      "Monday",
    ...
    const pullRequestsCreated = prEvents.filter(
      (e) => e.payload.action === "opened",
    ).length;
    const pullRequestsMerged = prEvents.filter(
      (e) => e.payload.action === "closed" && e.payload.pull_request?.merged,
    ).length;

    const issueEvents = yearEvents.filter((e) => e.type === "IssuesEvent");
    const issuesClosed = issueEvents.filter(
    ...
    const topCollaboratedRepo =
      Object.entries(repoActivityCount).sort((a, b) => b[1] - a[1])[0]?.[0] ||
      "N/A";
